...
const { gen, isValidUrl, isValidShortcode, minutesFromNow, coarseGeoFromIp } = require("./utils");
  ...
  try {
    ...
    if (validity !== undefined) {
      if (!Number.isInteger(validity) || validity <= 0) {
        await Log("backend", "error", "validation", "Validity must be a positive integer (minutes)");
    ...
    if (code !== undefined) {
      if (!isValidShortcode(code)) {
        await Log("backend", "error", "validation", "Invalid shortcode (alphanumeric, 4-20)");
        return res.status(400).json({ error: "Invalid shortcode (alphanumeric, 4-20)" });
      }
      if (exists(code)) {
